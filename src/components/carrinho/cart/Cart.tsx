...
function Cart() {
	...
	const { items, quantidadeItems, valorTotal, limparCarrinho } = useContext(CartContext);
			...
			<div className="container mx-auto px-4">
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
					...
					<div className="lg:col-span-2 space-y-4">
						{items.length === 0 ? (
							<div className="bg-white rounded-lg p-8 text-center text-gray-500 font-semibold shadow-sm">
								O seu carrinho está vazio.
							</div>
						) : (
							items.map((item) => (
								<CardCart key={item.produto.id} item={item} />
							))
						)}
					</div>
					...
					<div className="lg:col-span-1">
						<div className="bg-white rounded-lg shadow-sm p-6 sticky top-4">
							...
							<button
								...
								className="w-full bg-teal-500 hover:bg-teal-900 disabled:bg-slate-300 text-white font-semibold py-3 rounded-lg transition-colors cursor-pointer"
								type="button"
							>
								Finalizar Compra
							</button>
